...
if (isset($_POST['editar_factura'])) {
    ...
    $sql = "UPDATE facturas SET Productos=?, Cantidad=?, Precio=?, Nombre_servicio=?, Tipo_servicio=?, Precio_venta=?, Total=? WHERE ID_Factura=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sidsdsdi", $productos, $cantidad, $precio, $nombre_servicio, $tipo_servicio, $precio_venta, $total, $id_factura);
    $stmt->execute();

    header("Location: editar_factura.php?mensaje=actualizado");
    exit();
}
    ...
    <div class="user-panel-productos">
        ...
        <div class="pagination">
            <?php for ($i = 1; $i <= $totalPaginas; $i++): ?>
                <a href="?pagina=<?= $i ?>" class="<?= $paginaActual == $i ? 'active' : '' ?>"><?= $i ?></a>
            <?php endfor; ?>
        </div>
    </div>
</body>
